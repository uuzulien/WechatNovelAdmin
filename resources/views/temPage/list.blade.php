                ...
                <div class="panel-body">
                    <div class="table-responsive">
                        ...
                        @forelse($list as $key => $item)
                            <tr>
                                <td>{{$item->wechatnamecn}}</td>
                                <td>{{$item->wechatnameen}}</td>
                                <td>{{$item->wid}}</td>
                                <td>{{$item->title}}</td>
                                <td>{{$item->msg}}</td>
                                <td>{{$item->href}}</td>
                                ...
                                <td>
                                    ...
                                    <button type="button" class="btn btn-primary"  data-toggle="modal" data-target="#add_article" data-key="{{$key}}">修改</button>
...
@section('js')
    ...
    // 编辑逻辑
    var option = $("select[name='page_id']");
    $('#add_article').on('show.bs.modal', function (event) {
        ...
        if (data) {
            ...
            modal.find('#wechatnamecn').val(data.wechatnamecn);
            modal.find('#wechatnameen').val(data.wechatnameen);
            modal.find('#wid').val(data.wid);
            modal.find('#msg').val(data.msg);

        } else {
            modal.find('.modal-title').text('添加二跳公众号');
            modal.find('#wechatnamecn').val('');
            modal.find('#wechatnameen').val('');
            modal.find('#wid').val('');
            modal.find('#msg').val('');
        }
        for (var key in pages) {
            if (data) {
                option_content += `<option value="${key}" ${key == data['page_id'] ? 'selected' : ''}>${pages[key]}</option>`;
            } else {
                option_content += `<option value="${key}">${pages[key]}</option>`;
            }
            option_data.push(pages[key]);
        }
